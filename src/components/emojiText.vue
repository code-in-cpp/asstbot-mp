<template>
    <view class="emojiText">
        <block v-for="(item, index) in emojiNodes" :key="index">
            <block v-if="item.node == 'text'">
                <text>{{item.txt}}</text>
            </block>
            <block v-else-if="item.node == 'emoji'">
                <icon class="face" :class="item.txt"></icon>
            </block>
        </block>
    </view>
</template>
...
export default {
  ...
  computed: {
    emojiNodes () {
      if (!this.text) return []
      return emoji.parseText(this.text)
    }
  }
}
</script>
